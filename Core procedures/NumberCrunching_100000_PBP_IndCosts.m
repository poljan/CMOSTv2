...
% updated April 2018 for individual costs
...
function [y, Gender, DeathCause, Last, DeathYear, NaturalDeathYear,...
    ...
    PBP_Doc]...
    = NumberCrunching_100000_PBP_IndCosts(p, StageVariables, Location, Cost, CostStage, risc,...
...
PBP_Doc.Early     = ones(1, 100000)*-1; % number early adenomas
PBP_Doc.Advanced  = ones(1, 100000)*-1; % number advanced adenomas
PBP_Doc.Cancer    = ones(1, 100000)*-1; % number cancer
PBP_Doc.Screening = zeros(1, 100000);   % screening done?
...
Money.AllCost         = zeros(100, 100000);
Money.AllCostFuture   = zeros(100, 100000);
Money.Treatment       = zeros(100, 100000);
Money.FutureTreatment = zeros(100, 100000);
                        ...
                        if and(Included(z), ~isequal(ScreeningPreference(z), 0))
                            ...
                            if and(y >= ScreeningTest(preference, 4), y <ScreeningTest(preference, 5))
                                ...
                                if y-Last.Colonoscopy(z) >= ScreeningTest(preference, 7) % we only screen so and so many years after the last Colonoscopy
                                    if rand < ScreeningTest(preference, 2)
                                        if isequal(preference, 1) % Colonoscopy % corrected 23.04.2018
                                            ...
                                            if rand < ScreeningTest(preference, 2) % compliance with test
                                                ...
                                                if rand < Limit % if the test happens to be positive
                                                    if rand < ScreeningTest(preference, 3) % compliance with follow up
                                                        Number.Screening_Colonoscopy(y) = Number.Screening_Colonoscopy(y) + 1;
               % inactivated April 2018                     ScreeningPreference(z) = 1; % according to paper by Zauber et al. we need to continue with colonoscopy screening
                                                        [Polyp, Ca, Detected, Included, DeathCause, DeathYear,...
                                                            DiagnosedCancer, AdvancedPolypsRemoved, EarlyPolypsRemoved, Last, TumorRecord, PaymentType, Money, PBP_Doc]...
                                                ...
                                                elseif isequal(preference, 7)
                                                    ...
                                                    Money.Screening(y, z) = Money.Screening(y, z) + Cost.other;
                            ...
                            if isequal(y, PBP.Year)
                                if y-Last.Colonoscopy(z) >= 10
                                    if isequal(PBP.Mock, 1)
                                        ...
                                        PBP_Doc = Colonoscopy_Mock(z, Polyp,...
                                            Ca, StageVariables, Location, ColoReachMatrix, PBP_Doc);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
function PBP_Doc = Colonoscopy_Mock(z, Polyp,...
    Ca, StageVariables, Location, ColoReachMatrix, PBP_Doc)

PBP_Doc.Early(z) = 0; 
PBP_Doc.Advanced(z) = 0;
PBP_Doc.Cancer(z) = 0;
...
for f=length(find(Polyp.Polyps(z, :))) : -1 : 1
    ...
    if and(rand < StageVariables.Colo_Detection(Tumor) *...    % influence of polyp stage
        ...
        if Tumor > 4
            PBP_Doc.Advanced(z) = PBP_Doc.Advanced(z) +1; % for PBP
        else
            PBP_Doc.Early(z) = PBP_Doc.Early(z) +1; % for PBP
        end
    end
end
...
for f=length(find(Ca.Cancer(z, :))) : -1 : 1
    ...
    if and(rand < StageVariables.Colo_Detection(Tumor),...        % influence of cancer stage on detection
        ...
        PBP_Doc.Cancer(z) = PBP_Doc.Cancer(z) +1;  % for PBP
    end
end
...
switch Modus  %m2 moved the switch statement up. otherwise without cancer m remains 0
    case 'Scre' % Screening
        m=1;
    case {'PBPx', 'PBPm'}  % poor bowel preparateion
        m=1;
    case 'Symp' % Symptoms
        m=2;
    case 'Foll' % Follow-up
...
if or(isequal (Modus, 'PBPx'), isequal (Modus, 'PBPm'))
...
switch Modus
    case 'Scre' % Screening
        Money.Screening(y, z)    = Money.Screening(y, z) + moneyspent;
    case 'PBPx' % poor bowel preparation
        Money.Screening(y, z)    = Money.Screening(y, z) + moneyspent;
    case 'Symp' % Symptoms
        Money.Treatment(y, z)    = Money.Treatment(y, z) + moneyspent;
    case 'Foll' % Follow-up
        Money.FollowUp(y, z)     = Money.FollowUp(y, z) + moneyspent;
    case 'Base' % Baseline
        Money.Other(y, z)        = Money.Other(y, z) + moneyspent;
end

% if isequal (Modus, 'PBPm') % in mock mode
%     
%     Polyp                 = Polyp_tmp;
%     Ca                    = Ca_tmp;
%     Detected              = Detected_tmp;
%     Included              = Included_tmp;
%     DeathCause            = DeathCause_tmp;
%     DeathYear             = DeathYear_tmp;
%     DiagnosedCancer       = DiagnosedCancer_tmp;
%     AdvancedPolypsRemoved = AdvancedPolypsRemoved_tmp;
%     EarlyPolypsRemoved    = EarlyPolypsRemoved_tmp;
%     Last                  = Last_tmp;
%     TumorRecord           = TumorRecord_tmp;
%     PaymentType           = PaymentType_tmp;
%     Money                 = Money_tmp;
%     
% %    PBP_Doc is the only variable which will be modified here
% end
...
if isequal(counter, 0) % in this case no tumor or polyp
    Money.Screening(y, z) = Money.Screening(y, z) + Cost.Sigmoidoscopy;
    PaymentType.RS(1,y)= PaymentType.RS(1,y)+1;  %m2 assumption. RS is only done for screening. not for followup or treatment
else
    Money.Screening(y, z) = Money.Screening(y, z) + Cost.Sigmoidoscopy_Polyp;
...
if rand < risc.Rectosigmo_Perforation
    % a perforation happend
    Money.Screening(y, z) = Money.Screening(y, z) + Cost.Colonoscopy_Perforation;
...
for x1 = 1:100
    for x2= 1:4
        Money.Treatment(x1, z)       = Money.Treatment(x1, z) + SubCostAll(Counter);
        Money.FutureTreatment(x1, z) = Money.FutureTreatment(x1, z) + SubCostAllFut(Counter);
        Counter = Counter + 1;
    end
end
