...
function [data,BM] = Evaluation(data, Variables)
...
SummaryVariable{17,1} = sum(data.Money.AllCost(1:100));
...
String{10}=sprintf('%g years lost to colo', sum(data.NaturalDeathYear(data.DeathCause==3)...
                                                - data.DeathYear(data.DeathCause==3)));
String{11}=sprintf('%g total CRR rel costs', sum(data.Money.AllCost));                                            
...
if DispFlag
    ...
    plot(0:99, data.Money.AllCost(1:100)/n, 'g'),  hold on
    plot(0:99, data.Money.Treatment(1:100)/n, 'k')
    plot(0:99, data.Money.FollowUp(1:100)/n, 'b')
    plot(0:99, data.Money.Screening(1:100)/n, 'r')
...
if ExcelFlag
    ...
    if isequal(Variables.StarterFlag, 'on')
        ...
        [Succ, Msg]=xlswrite(FileName, reshape(round(data.Money.AllCost(1:100)/n*100)/100, 100, 1), 'Costs', HeadLineColumns_2{Counter}); %#ok<ASGLU>
        ...
        if isequal(answer, 'Yes')
            if ExcelFlag
                ...
                [Succ, Msg]=xlswrite(FileName, reshape(round(data.Money.AllCost(1:100)/n*100)/100, 100, 1), 'Summary', 'F2'); %#ok<ASGLU>
...
if ResultsFlag
    ...
    Results.Treatment       = reshape(round(data.Money.Treatment(1:100)/n*100)/100, 100, 1);
    Results.TreatmentFuture = reshape(round(data.Money.FutureTreatment(1:100)/n*100)/100, 100, 1); 
    Results.Screening       = reshape(round(data.Money.Screening(1:100)/n*100)/100, 100, 1);
    Results.FollowUp        = reshape(round(data.Money.FollowUp(1:100)/n*100)/100, 100, 1);
    Results.Other           = reshape(round(data.Money.Other(1:100)/n*100)/100, 100, 1);
