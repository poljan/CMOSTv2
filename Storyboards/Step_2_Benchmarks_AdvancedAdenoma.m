...
function varargout = Step_2_Benchmarks_AdvancedAdenoma(varargin)

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Step_2_Benchmarks_AdvancedAdenoma_OpeningFcn, ...
                   'gui_OutputFcn',  @Step_2_Benchmarks_AdvancedAdenoma_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Step_2_Benchmarks_AdvancedAdenoma is made visible.
function Step_2_Benchmarks_AdvancedAdenoma_OpeningFcn(hObject, eventdata, handles, varargin)
...
handles.Ad_distr_handles = {'ad_distr_1'; 'ad_distr_2'; 'ad_distr_3'; 'ad_distr_4'; 'ad_distr_5'; 'ad_distr_6'};
...
for f= (length(handles.Benchmarks.AdvPolyp.Ov_y) + 1) : length(handles.Ov_y_handles)
    handles.Benchmarks.AdvPolyp.Ov_y(f) = 0;
end
for f= (length(handles.Benchmarks.AdvPolyp.Ov_perc) + 1) : length(handles.Ov_perc_handles)
    handles.Benchmarks.AdvPolyp.Ov_perc(f) = 0;
end
for f= (length(handles.Benchmarks.AdvPolyp.Male_y) + 1) : length(handles.Male_y_handles)
    handles.Benchmarks.AdvPolyp.Male_y(f) = 0;
end
for f= (length(handles.Benchmarks.AdvPolyp.Male_perc) + 1) : length(handles.Male_perc_handles)
    handles.Benchmarks.AdvPolyp.Male_perc(f) = 0;
end
for f= (length(handles.Benchmarks.AdvPolyp.Female_y) + 1) : length(handles.Female_y_handles)
    handles.Benchmarks.AdvPolyp.Female_y(f) = 0;
end
for f= (length(handles.Benchmarks.AdvPolyp.Female_perc) + 1) : length(handles.Female_perc_handles)
    handles.Benchmarks.AdvPolyp.Female_perc(f) = 0;
end

[~,vv] = sort(handles.Benchmarks.AdvPolyp.Ov_y);
handles.Benchmarks.AdvPolyp.Ov_y = handles.Benchmarks.AdvPolyp.Ov_y(vv);
handles.Benchmarks.AdvPolyp.Ov_perc  = handles.Benchmarks.AdvPolyp.Ov_perc(vv);
[~,vv] = sort(handles.Benchmarks.AdvPolyp.Male_y);
handles.Benchmarks.AdvPolyp.Male_y = handles.Benchmarks.AdvPolyp.Male_y(vv);
handles.Benchmarks.AdvPolyp.Male_perc  = handles.Benchmarks.AdvPolyp.Male_perc(vv);
[~,vv] = sort(handles.Benchmarks.AdvPolyp.Female_y);
handles.Benchmarks.AdvPolyp.Female_y = handles.Benchmarks.AdvPolyp.Female_y(vv);
handles.Benchmarks.AdvPolyp.Female_perc  = handles.Benchmarks.AdvPolyp.Female_perc(vv);
...
for f=1:length(handles.Ov_y_handles)
    set(handles.(handles.Ov_y_handles{f}), 'string', num2str(handles.Benchmarks.AdvPolyp.Ov_y(f)));
end
for f=1:length(handles.Ov_perc_handles)
    set(handles.(handles.Ov_perc_handles{f}), 'string', num2str(handles.Benchmarks.AdvPolyp.Ov_perc(f)));
end

% adjusting numbers male
for f=1:length(handles.Male_y_handles)
    set(handles.(handles.Male_y_handles{f}), 'string', num2str(handles.Benchmarks.AdvPolyp.Male_y(f)));
end
for f=1:length(handles.Male_perc_handles)
    set(handles.(handles.Male_perc_handles{f}), 'string', num2str(handles.Benchmarks.AdvPolyp.Male_perc(f)));
end

% adjusting numbers female
for f=1:length(handles.Female_y_handles)
    set(handles.(handles.Female_y_handles{f}), 'string', num2str(handles.Benchmarks.AdvPolyp.Female_y(f)));
end
for f=1:length(handles.Female_perc_handles)
    set(handles.(handles.Female_perc_handles{f}), 'string', num2str(handles.Benchmarks.AdvPolyp.Female_perc(f)));
end

% Adjusting graph overall
axes(handles.Ov_plot); cla(handles.Ov_plot) %#ok<*MAXES>
XX = handles.Benchmarks.AdvPolyp.Ov_y;
YY = handles.Benchmarks.AdvPolyp.Ov_perc;
vv = find(XX==0);
XX(vv)=[];
YY(vv)=[];
...
XX = handles.Benchmarks.AdvPolyp.Male_y;
YY = handles.Benchmarks.AdvPolyp.Male_perc;
vv = find(XX==0);
XX(vv)=[];
YY(vv)=[];
...
XX = handles.Benchmarks.AdvPolyp.Female_y;
YY = handles.Benchmarks.AdvPolyp.Female_perc;
vv = find(XX==0);
XX(vv)=[];
YY(vv)=[];
...
axes(handles.Ad_distribution_plot); cla(handles.Ad_distribution_plot) %#ok<*MAXES>
XX = [1 2 3 4 5 6];
...
function varargout = Step_2_Benchmarks_AdvancedAdenoma_OutputFcn(hObject, eventdata, handles) 
...
if succ, if num >=0, handles.Benchmarks.AdvPolyp.Ov_perc(c)=num; end, end
...
if succ, if num >=0, handles.Benchmarks.AdvPolyp.Ov_y(c)=num; end, end
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>
...
if succ, if num >=0, handles.Benchmarks.AdvPolyp.Male_perc(c)=num; end, end
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>
...
if succ, if num >=0, handles.Benchmarks.AdvPolyp.Male_y(c)=num; end, end
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>
...
if succ, if num >=0, handles.Benchmarks.AdvPolyp.Female_perc(c)=num; end, end
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>
...
if succ, if num >=0, handles.Benchmarks.AdvPolyp.Female_y(c)=num; end, end
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>
...
function ad_distr_1_CreateFcn(hObject, eventdata, handles)
function ad_distr_2_CreateFcn(hObject, eventdata, handles)
function ad_distr_3_CreateFcn(hObject, eventdata, handles)
function ad_distr_4_CreateFcn(hObject, eventdata, handles)
function ad_distr_5_CreateFcn(hObject, eventdata, handles)
function ad_distr_6_CreateFcn(hObject, eventdata, handles)

function ad_distr_1_Callback(hObject, eventdata, handles)
...
elseif isequal(Answer, 'Yes')
    ...
    handles.Benchmarks.AdvPolyp.Ov_y     = handles.Benchmarks.AdvPolyp.Ov_y(vv);
    handles.Benchmarks.AdvPolyp.Ov_perc  = handles.Benchmarks.AdvPolyp.Ov_perc(vv);
    for f=length(handles.Benchmarks.AdvPolyp.Ov_y):-1:1
        if isequal(handles.Benchmarks.AdvPolyp.Ov_y(f), 0)
            handles.Benchmarks.AdvPolyp.Ov_y(f) = [];
            handles.Benchmarks.AdvPolyp.Ov_perc(f) = [];
        end
    end
    ...
    handles.Benchmarks.AdvPolyp.Male_y     = handles.Benchmarks.AdvPolyp.Male_y(vv);
    handles.Benchmarks.AdvPolyp.Male_perc  = handles.Benchmarks.AdvPolyp.Male_perc(vv);
    for f=length(handles.Benchmarks.AdvPolyp.Male_y):-1:1
        if isequal(handles.Benchmarks.AdvPolyp.Male_y(f), 0)
            handles.Benchmarks.AdvPolyp.Male_y(f) = [];
            handles.Benchmarks.AdvPolyp.Male_perc(f) = [];
        end
    end
    ...
    handles.Benchmarks.AdvPolyp.Female_y = handles.Benchmarks.AdvPolyp.Female_y(vv);
    handles.Benchmarks.AdvPolyp.Female_perc  = handles.Benchmarks.AdvPolyp.Female_perc(vv);
    for f=length(handles.Benchmarks.AdvPolyp.Female_y):-1:1
        if isequal(handles.Benchmarks.AdvPolyp.Female_y(f), 0)
            handles.Benchmarks.AdvPolyp.Female_y(f)    = [];
            handles.Benchmarks.AdvPolyp.Female_perc(f) = [];
        end
    end
